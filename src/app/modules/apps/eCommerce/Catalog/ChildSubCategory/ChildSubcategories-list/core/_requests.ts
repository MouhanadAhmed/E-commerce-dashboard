...
const getChildSubCategoryById = (
  id: ID,
): Promise<ChildSubCategories | undefined> => {
  return axios
    .get(`${CHILD_SUB_CATEGORY_URL}/${id}`)
    .then(
      (response: AxiosResponse<Response<ChildSubCategories>>) => response.data,
    )
...
const updateChildSubCategoryOrder = (
  CategoryId: ID,
  Order: number,
): Promise<ChildSubCategories | undefined> => {
  return axios
...
const deleteSelectedChildSubCategories = (
  userIds: Array<ID>,
): Promise<void> => {
  const requests = userIds.map((id) =>
    axios.delete(`${CHILD_SUB_CATEGORY_URL}/${id}`),
  );
...
const updateProductOrderInChildSubCategory = (
  categoryId: string,
  productId: string,
  order: number,
): Promise<[]> => {
  return axios.put(`${API_URL}/product/childSubCategory/${productId}`, {
    order: order,
    ChildSubCategory: categoryId,
  });
};
