...
const updateProduct = (CategoryId: string| undefined,Category: object): Promise<Product | undefined> => {
  return axios
...
const updateProductOrder = (CategoryId: ID,Order: number): Promise<Product | undefined> => {
  return axios
...
const updateSelectedProducts = (userIds: Array<ID>,Category: object): Promise<void> => {
  const requests = userIds.map((id) => axios.put(`${PRODUCT_URL}/${id}`, Category));
  return axios.all(requests).then(() => {});
};
