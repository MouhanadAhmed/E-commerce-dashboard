...
const innerText = tempElement.textContent || tempElement.innerText;
      ...
      },
      {
        ...
        Cell: ({ cell }) => (
          // <span className={`badge ${cell.getValue<boolean>() == true?'badge-success':'badge-danger'}`}>{cell.getValue<number>().toLocaleString()}</span>
          <div className="form-check form-switch form-check-custom form-check-solid">
          <input className="form-check-input cursor-pointer" type="checkbox" checked={cell.getValue<boolean>()} onClick={()=> updateCategoryAvailable.mutateAsync({id:cell.row.original._id,update:{showWeight:!cell.row.original.showWeight}})} id={cell.row.original._id}/>
          {/* {console.log('cell',cell.row.original)} */}
      </div>
      ...
      },
      {
        ...
        Edit: ({cell, row, table}) => {
          const childsubcategories = cell.getValue<{ childSubCategory: { name: string , _id:string} }[]>();
          // console.log("subcategories from child",subcategories)
          let defV =[]
          ...
          return (
            ...
            // ) : (
              <Select
                ...
                onChange={(selected) => {
                  const ChildSubCategories = selected ? selected.map((option) => option.value  ) : [];
                //  console.log('updatedBranches',updatedBranches)
        ...
        Cell: ({ cell }) => {
          const categoriess = cell.getValue<{ childSubCategory: { name: string } }[]>();
          // console.log('categoriess',categoriess)
          return (
            <>
              {categoriess?.map(childSubCategory => <span className="badge badge-primary me-1">
      ...
      },
      {
        ...
        Edit: ({cell, row, table}) => {
          const childsubcategories = {value:cell.getValue(),label:cell.getValue()};
          // console.log("subcategories from child",subcategories)
          let defV =[]
          // childsubcategories.map(childSubCategory => {
          //   defV.push({value:childSubCategory?.childSubCategory?._id, label:childSubCategory?.childSubCategory?.name})
    ...
    [validationErrors, memoizedBranches, memoizedCategories, memoizedSubCategories, memoizedChildSubCategories, memoizedExtras, memoizedTypes],
  );
  ...
  const handleSaveCategories  = async (originalRow) => {
    ...
    const updatedRowValues = {
      ...
      childSubCategory: editChildSubCategory?.map(str => ({ childSubCategory: str })),
  ...
  const deleteItem = useMutation(() => deleteProduct(CategoriesDelete as string), {
  ...
  const table1 = useMaterialReactTable({
    ...
    renderDetailPanel: ({ row }) =>
      row.original.branch ? (
        ...
        <div>
          childSubCategories : 
        {row.original.childSubCategory.map((childSubCategory,index) => <span key={index} className="badge badge-primary me-1">
              {childSubCategory?.childSubCategory?.name}
            </span>)}
          </div>
        {/* <div>
