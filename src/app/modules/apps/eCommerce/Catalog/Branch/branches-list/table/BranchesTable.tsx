...
import { deleteBranch, deleteSelectedBranches, updateBranch } from '../core/_requests';
...
const BranchesTable = () => {
  ...
  const [archivedBranches, setArchivedBranches] = useState<Branch[]>(() => archived);
  ...
  const deleteSelectedItems = useMutation((ids:string[]) => deleteSelectedBranches(ids), {
    // 💡 response of the mutation is passed to onSuccess
    onSuccess: () => {
      // ✅ update detail view directly
      queryClient.invalidateQueries([`${QUERIES.BRNACHES_LIST}`])
  ...
  const updateBranchAvailable = useMutation(({id,update}) => updateBranch(id,update), {
    // 💡 response of the mutation is passed to onSuccess
    onSuccess: () => {
      // ✅ update detail view directly
      queryClient.invalidateQueries([`${QUERIES.BRNACHES_LIST}`]);
